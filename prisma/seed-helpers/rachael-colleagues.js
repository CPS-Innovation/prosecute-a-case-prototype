...
const UNIT_IDS = [3, 4]; // Wessex Crown Court, Wessex RASSO

async function seedRachaelColleagues(prisma) {
  ...
  for (let i = 0; i < 20; i++) {
    ...
    colleaguesData.push({
      email: `${firstName.toLowerCase()}.${lastName.toLowerCase()}.rh-p${i}@example.com`,
  ...
  for (let i = 0; i < 20; i++) {
    ...
    const lastName = faker.helpers.arrayElement(lastNames);
    colleaguesData.push({
      email: `${firstName.toLowerCase()}.${lastName.toLowerCase()}.rh-po${i}@example.com`,
      password: 'password123',
...
module.exports = { seedRachaelColleagues };
