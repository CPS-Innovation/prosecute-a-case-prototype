...
const UNIT_IDS = [3]; // Wessex Crown Court

async function seedKirstyColleagues(prisma) {
  ...
  for (let i = 0; i < 20; i++) {
    ...
    colleaguesData.push({
      email: `${firstName.toLowerCase()}.${lastName.toLowerCase()}.kp-p${i}@example.com`,
  ...
  for (let i = 0; i < 20; i++) {
    ...
    const lastName = faker.helpers.arrayElement(lastNames);
    colleaguesData.push({
      email: `${firstName.toLowerCase()}.${lastName.toLowerCase()}.kp-po${i}@example.com`,
      password: 'password123',
...
module.exports = { seedKirstyColleagues };
