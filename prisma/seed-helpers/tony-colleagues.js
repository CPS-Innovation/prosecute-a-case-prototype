...
const UNIT_IDS = [1, 2, 3, 4, 5, 6, 7]; // All Wessex units

async function seedTonyColleagues(prisma) {
  ...
  for (let i = 0; i < 20; i++) {
    ...
    colleaguesData.push({
      email: `${firstName.toLowerCase()}.${lastName.toLowerCase()}.ts-p${i}@example.com`,
  ...
  for (let i = 0; i < 20; i++) {
    ...
    const lastName = faker.helpers.arrayElement(lastNames);
    colleaguesData.push({
      email: `${firstName.toLowerCase()}.${lastName.toLowerCase()}.ts-po${i}@example.com`,
      password: 'password123',
...
module.exports = { seedTonyColleagues };
