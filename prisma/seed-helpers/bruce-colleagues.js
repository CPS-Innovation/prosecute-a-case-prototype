...
async function seedBruceColleagues(prisma) {
  ...
  for (let i = 0; i < 20; i++) {
    ...
    colleaguesData.push({
      email: `${firstName.toLowerCase()}.${lastName.toLowerCase()}.bb-p${i}@example.com`,
  ...
  for (let i = 0; i < 20; i++) {
    ...
    const lastName = faker.helpers.arrayElement(lastNames);
    colleaguesData.push({
      email: `${firstName.toLowerCase()}.${lastName.toLowerCase()}.bb-po${i}@example.com`,
      password: 'password123',
...
module.exports = { seedBruceColleagues };
